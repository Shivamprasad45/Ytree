      ...
      <div className="max-w-[1200px] mx-auto px-6">
        <div className="flex flex-col md:flex-row justify-between items-end gap-6 mb-16">
          <div className="max-w-2xl">
            <h2 className="text-deep-forest text-4xl md:text-5xl font-black leading-tight mb-4">
              Regenerating the planet is simpler than you think.
            </h2>
            <p className="text-sage text-lg font-medium">Making a tangible difference is woven into every interaction with Vanagrow.</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {steps.map((step) => (
            <div 
              key={step.id} 
              className="group flex flex-col gap-6 rounded-3xl bg-white p-10 shadow-sm border border-transparent hover:border-primary/20 hover:shadow-xl transition-all"
            >
              ...
              <div className="flex flex-col gap-3">
                <h3 className="text-deep-forest text-2xl font-bold">{step.id}. {step.title}</h3>
                <p className="text-sage text-base leading-relaxed">{step.description}</p>
              </div>
            </div>
          ))}
        </div>
